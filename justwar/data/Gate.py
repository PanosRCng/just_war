...
class Gate(GameElement):
	...
	def __init__(self, direction):
		...
		self.coord = ( randint(Config.screenWidth-900, Config.screenWidth-200), 0 ) 

		if direction == 1:
			self.shape = pygame.transform.rotate(self.shape, -90)
			self.coord = ( Config.screenWidth-self.shape.get_width()-10, randint(Config.screenHeight-540, Config.screenHeight-200) )

		if direction == 2:
			self.shape = pygame.transform.rotate(self.shape, -180)
			self.coord = ( randint(Config.screenWidth-900, Config.screenWidth-200), Config.screenHeight-self.shape.get_height() )

		if direction == 3:
			self.shape = pygame.transform.rotate(self.shape, 90)
			self.coord = ( 0+10, randint(Config.screenHeight-540, Config.screenHeight-200) )
